let _Timeout = function (_cb: Function, _ms: number, _isRepeat: boolean, _args: any[]) {
  ...
  class Timeout {
    ...
    constructor(cb: Function, ms: number, isRep: boolean, args: any[]) {
      this._ = { t: 0 as any, f: cb, m: (ms = (ms = +ms) > 0 ? ms : 1), i: isRep, a: args, r: true }
      loop(this, ms)
    }
...
function longSetTimeout<F extends (...a: any[]) => any>(
  ...
  return new _Timeout(callback, ms, false, args) as ITimeout
}
...
function longSetInterval<F extends (...a: any[]) => any>(
  ...
  return new _Timeout(callback, ms, true, args) as ITimeout
}
